...
namespace AS_CRM.Controllers
{
    public class PTareasController : CustomController
    {
        ...
        public ActionResult Create(int? ido)
        {
            ...
            string _idUserNa = db.AspNetUsers.Where(w => w.Email == "NA@alusoft.com.ar").ToList<AspNetUser>().ElementAt(0).Id;

            int _Proyecto_id = db.PObjetivos.Find(ido).Proyecto_Id.Value;
            ViewBag.ido = _Proyecto_id;

            ViewBag.Usuario_Id = new SelectList(db.AspNetUsers, "Id", "UserName",_idUserNa);
        ...
        public ActionResult Create([Bind(Include = "Id,Nombre,Usuario_Id,Estado_Id,Objetivo_Id,Sprint_Id,FechaIncio,FechaFinalizado,FechaEntrega,HorasEstimadas,Detalle,Prioridad_Id,Certificada")] PTarea pTarea)
        {
            if (!validarLoggin())
                return RedirectToAction("Index", "Home");
        ...
        // GET: PTareas/Create
        public ActionResult CreateModal(int? ido)
        {
            if (!validarLoggin())
                return RedirectToAction("Index", "Home");
            ...
            int _Proyecto_id = db.PObjetivos.Find(ido.Value).Proyecto_Id.Value;
            ViewBag.ido = _Proyecto_id;

            ViewBag.Usuario_Id = new SelectList(db.AspNetUsers, "Id", "UserName", _idUserNa);
            ViewBag.Estado_Id = new SelectList(db.PEstados, "Id", "Nombre");
            ViewBag.Objetivo_Id = new SelectList(db.PObjetivos.Where(w => w.Proyecto_Id==_Proyecto_id), "Id", "Nombre");
            ViewBag.Sprint_Id = new SelectList(db.PSprints, "Id", "Nombre");
            ViewBag.Prioridad_Id = new SelectList(db.PPrioridades, "Id", "Nombre");
            return View();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateModal([Bind(Include = "Id,Nombre,Usuario_Id,Estado_Id,Objetivo_Id,Sprint_Id,FechaIncio,FechaFinalizado,FechaEntrega,HorasEstimadas,Detalle,Prioridad_Id,Certificada")] PTarea pTarea)
        {
            if (!validarLoggin())
                return RedirectToAction("Index", "Home");
            ...
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                var _idp = db.PObjetivos.Find(pTarea.Objetivo_Id).Proyecto_Id.Value;
                return RedirectToAction("Tareas", "PObjetivoes", new { idp = _idp });
            }
        ...
        public ActionResult Edit(int? id)
        {
            ...
            ViewBag.Usuario_Id = new SelectList(db.AspNetUsers, "Id", "UserName", pTarea.Usuario_Id);
            ViewBag.Estado_Id = new SelectList(db.PEstados, "Id", "Nombre", pTarea.Estado_Id);
            ViewBag.Objetivo_Id = new SelectList(db.PObjetivos.Where(w=> w.Proyecto_Id == pTarea.PObjetivo.Proyecto_Id), "Id", "Nombre", pTarea.Objetivo_Id);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,Nombre,Usuario_Id,Estado_Id,Objetivo_Id,Sprint_Id,FechaIncio,FechaFinalizado,FechaEntrega,HorasEstimadas,Detalle, Prioridad_Id,Certificada")] PTarea pTarea)
        {
            if (ModelState.IsValid)
            {
        ...
        public ActionResult EditModal(int? id, string resp)
        {
            if (!validarLoggin())
                return RedirectToAction("Index", "Home");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditModal([Bind(Include = "Id,Nombre,Usuario_Id,Estado_Id,Objetivo_Id,Sprint_Id,FechaIncio,FechaFinalizado,FechaEntrega,HorasEstimadas,Detalle, Prioridad_Id,Certificada")] PTarea pTarea, string resp)
        {
            if (ModelState.IsValid)
            {
                db.Entry(pTarea).State = EntityState.Modified;
                ...
                if (resp.Equals("cert"))
                {
                    var _obj = db.PObjetivos.Find(pTarea.Objetivo_Id);
                    return RedirectToAction("Details", "Proyectoes", new { id = _obj.Proyecto_Id});
                }
                else 
                {
